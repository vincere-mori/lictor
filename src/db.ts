...
export async function addTaskFromText(text: string, now = Date.now()) {
  ...
  await db.events.add({ taskId: task.id, type: 'created', tier: task.tier, ts: now })
  return task
}
...
const seed: () => Task[] = () => {
  const now = Date.now()
  return [
    { id: uid(), title: 'Прочитать 20 страниц', tier: 'MONEO', due: now + 7 * 3600e3, status: 'active', createdAt: now, snoozes: 0 },
    { id: uid(), title: 'Позвонить в деканат', tier: 'INSTO', due: now + 25 * 60e3, status: 'active', createdAt: now, snoozes: 0 },
    { id: uid(), title: 'Сдать лабу ИТМО', tier: 'COGO', due: now - (12 * 60e3 + 24e3), status: 'active', createdAt: now, snoozes: 0 }
  ]
}
